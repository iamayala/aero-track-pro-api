...
exports.post=(req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      ...
      res.status(201).json({ id: result.insertId, part_id, quantity, order_date, expected_delivery_date, status });
    }
  );
};
...
exports.get = (req, res) => {
  db.query('SELECT Orders.*, Parts.* FROM Orders JOIN Parts ON Orders.part_id = Parts.id', (err, results) => {
    if (err) {
...
exports.getOne = (req, res) => {
  ...
  db.query('SELECT Orders.*, Parts.* FROM Orders JOIN Parts ON Orders.part_id = Parts.id WHERE Orders.id = ?', [id], (err, results) => {
    if (err) {
