...
exports.post = (req, res) => {
  ...
  db.query('SELECT * FROM Aircrafts WHERE registration_number = ?', [registration_number], (selectErr, selectResults) => {
    if (selectErr) {
    ...
    if (selectResults.length > 0) {
      return res.status(400).json({ message: 'Aircraft with this registration number already exists' });
    }
    ...
    db.query(
      ...
      [registration_number, manufacturer, model, year_of_manufacture, capacity, fuel_capacity, max_speed, max_range, current_location, status],
      (insertErr, results) => {
        if (insertErr) {
          console.error('Error creating aircraft:', insertErr);
          return res.status(500).json({ message: 'Internal server error' });
        }
        res.status(201).json({ message: 'Aircraft created successfully', id: results.insertId });
      }
    );
  });
};
...
exports.put = (req, res) => {
  ...
  db.query(
    ...
    [registration_number, manufacturer, model, year_of_manufacture, capacity, fuel_capacity, max_speed, max_range, current_location, status, id],
    (err, results) => {
      if (err) {
